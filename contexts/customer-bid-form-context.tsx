...
function hasInvalidFields(fields: IBidFields): string | undefined {
  if (!fields.name) return "Name is required.";
  if (!fields.products.length) return "At least one product is required";
  if (fields.hoa_approval_required) {
    if (!fields.hoa_contact_name) return "Contact name is required";
    if (!fields.hoa_contact_email || !fields.hoa_contact_phone)
      return "Contact email or phone is required";
  }
  if (fields.has_water_rebate && !fields.water_rebate_company)
    return "Water rebate company name is required";

  if (fields.media.length) {
    const mediaFieldTypes = fields.media.map((m) => m.type);
    const missingRequiredMediaTypes = Object.entries(MEDIA_TYPES).flatMap(
      ([typeKey, type]) => {
        if (!type.required) return [];
        if (mediaFieldTypes.includes(typeKey as MEDIA_TYPES_KEYS)) return [];
        return typeKey;
      }
    );
    ...
    if (missingRequiredMediaTypes.length) {
      ...
      return `Missing media: ${missingRequiredMediaTypeStringParts.join(
        ", "
      )} is required`;
    }
  } else {
...
export function CustomerBidFormContextProvider(props: PropsWithChildren) {
  ...
  const isFormInvalid = hasInvalidFields(formState.fields);

  const handleSubmit = useCallback(async () => {
    if (isFormInvalid) {
      return dispatch({
        type: FormReducerActionType.SET_ERROR,
        payload: isFormInvalid,
      });
    }
  ...
  const value = useMemo(
    () => ({
      ...
      formState,
      handleSubmit,
      isFormInvalid: hasInvalidFields(formState.fields),
      preview,
      products,
